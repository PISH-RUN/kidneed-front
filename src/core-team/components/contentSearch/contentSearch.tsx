...
export const ContentSearch = ({ type, value, onChange }: any) => {
  ...
  useEffect(() => {
    const handler: NodeJS.Timeout = setTimeout(() => {
      setOptions([]);
      getContents({ search, type }).then(resp => setOptions(resp?.data?.map((content: any) => ({
        value: content.id,
  ...
  return (
    <div className="tw-flex tw-items-center">
      ...
      />
      <MdInfo onClick={() => setContent(value)} className={`${!value ? 'tw-text-gray-400' : 'tw-text-blue-400 tw-cursor-pointer'} tw-text-3xl tw-mr-2`} />
      <ContentDetail
